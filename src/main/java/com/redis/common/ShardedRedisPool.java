...
public class ShardedRedisPool {
    ...
    private static Logger logger = LoggerFactory.getLogger(ShardedRedisPool.class);

    //Jedis连接池MAP
    private static final Map<String, ShardedJedisPool> POOL_CACHE = new Hashtable<>();
    ...
    private void load(String source) {
        ...
        logger.debug("REDIS_HOST=" + HOST + ";REDIS_PORT=" + PORT + ";REDIS_PASSWORD=" + PASSWORD);
        ...
        ShardedJedisPool pool = new ShardedJedisPool(config, infos, Hashing.MURMUR_HASH, Sharded.DEFAULT_KEY_TAG_PATTERN);
     ...
     * @param jedis
     */
    public void releaseSession(ShardedJedis jedis) {
        if (jedis != null) {
            jedis.close();
        }
    }
